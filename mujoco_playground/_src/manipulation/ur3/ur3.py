...
_GRIPPER_JOINTS = ["finger_joint"]


def get_assets() -> Dict[str, bytes]:
    assets = {}
    path = mjx_env.ROOT_PATH / "manipulation" / "ur3" / "xmls"
...
class Ur3Base(mjx_env.MjxEnv):
  ...
  def _post_init(self, obj_name: str = "object_box", keyframe: str = "home"):
    ...
    self._gripper_site = self._mj_model.site("pinch_site").id

    self._left_finger_geom = self._mj_model.geom("left_pad").id
    self._right_finger_geom = self._mj_model.geom("right_pad").id

    self._hand_geom = self._mj_model.geom("v_gripper_base").id
